  ...
  >
    ...
    >
      ...
      <ul class="dark:text-white p-8">
        ...
        >
          <span class="flex items-center justify-between w-full space-x-3">
            ...
            <span>${{ formatCurrenty(product.price / 100) }}</span>
            ...
            <span class="w-16 text-right">
              ${{ formatCurrenty((product.price * product.quantity) / 100) }}
            </span>
          </span>
        </li>
      </ul>
      <p class="flex items-center justify-between text-3xl mt-4 p-8">
        <span>Total:</span>
        <span class="font-bold">${{ formatCurrenty(total) }}</span>
      </p>
    </div>
  </div>
</template>
